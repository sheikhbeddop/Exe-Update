...
class api:
    ...
    def __init__(self, name, ownerid, version, hash_to_check):
        ...
        self.app_data = self.application_data_class()
        self.init()
    ...
    def init(self):
        if self.sessionid != "":
            raise Exception("KeyAuth application is already initialized.")
        ...
        if response == "KeyAuth_Invalid":
            raise Exception("The application does not exist on the KeyAuth dashboard.")
        ...
        if json["message"] == "invalidver":
            if json["download"] != "":
                ...
                raise Exception("New version available. Download link opened in browser.")
            else:
                raise Exception("Invalid version. Please contact the owner.")
    ...
    def checkinit(self):
        if not self.initialized:
            raise Exception("Initialize first, in order to use the functions")
    ...
    def __do_request(self, post_data):
        try:
            ...
            if not signature or not timestamp:
                raise Exception("Missing headers for signature verification. KeyAuth server connection failed.")
            ...
            if time_difference > timedelta(seconds=20 + buffer_seconds):
                raise Exception("Timestamp verification failed (time difference is too large).")
            ...
            if not verify_key(response.text.encode('utf-8'), signature, timestamp, '5586b4bc69c7a4b487e4563a4cd96afd39140f919bd31cea7d1c6a1e8439422b'):
                raise Exception("Signature checksum failed. Request was tampered with or session ended.")

            return response.text

        except requests.exceptions.Timeout: 
            raise Exception("Request timed out. KeyAuth server is down or slow.")
